...
int main()
{
  ...
  for(int i=0;i<M;i++)
  {
    ...
    temp.hash=temp.in_h*3600+temp.in_m*60+temp.in_s;
    if(temp.hash<min.hash) min=temp;
    temp.hash=temp.out_h*3600+temp.out_m*60+temp.out_s;
    if(temp.hash>max.hash) max=temp;
  }
