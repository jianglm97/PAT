...
A given order is a permutation of distinct integers in [1, 54]. If the number at the i-th position is j, it means to move the card from position i to position j. For example, suppose we only have 5 cards: S3, H5, C1, D13 and J2.
...
int main()
{
  ...
  int K;
  scanf("%d",&K);
  for(int i=1;i<=54;i++) scanf("%d",&s[i]);
  for(int j=1;j<=K;j++)
  {
    for(int i=1;i<=54;i++)
    {
  ...
  for(int i=1;i<=53;i++) printf("%c%d ",c[K][i],n[K][i]);
  printf("%c%d",c[K][54],n[K][54]);
  return 0;
}
